...
React + TypeScript + Viteで作成した、スマホ対応の領収書管理Webアプリです。領収書画像をVercel Function経由でOpenAI APIへ送り、店舗名・金額・日付を抽出します。APIキーはブラウザへ配信されません。

## 主な機能

- レポート一覧、作成、詳細画面
- スマホカメラ撮影、画像選択、プレビュー、送信用画像圧縮
- OpenAI Responses APIとStructured Outputsによる領収書解析
...
- APIキー自体はVercel Function内に保持されますが、匿名公開APIは第三者から呼び出される可能性があります。Vercel Firewallのレート制限や認証を追加し、OpenAI側の使用量上限も設定してください。
- 領収書画像は解析時にOpenAI APIへ送信されます。個人情報を含む画像の取り扱い方針を利用者へ明示してください。
